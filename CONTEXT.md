...
### 2026-07-11 (sesión 18)
- Qué cambió: pedido del usuario — en la pestaña Quests, ordenar mostrando las últimas editadas primero (antes no había ningún sort explícito: `renderQuests()` mostraba `DB.quests` tal cual venía de `localStorage`, es decir en orden de creación). Se agregó, en `app.js`, `quests.sort((a, b) => new Date(b.updatedAt || b.createdAt) - new Date(a.updatedAt || a.createdAt))` justo después de aplicar los 3 filtros existentes (estado, categoría, búsqueda) y antes de armar el HTML. `updatedAt` es opcional en el modelo de Quest — `saveQuest()` sólo lo setea en la rama de edición (`if (editId)`), nunca al crear —, así que una quest jamás editada cae al fallback de su `createdAt`, que sí es obligatorio. A diferencia de `buildDashWaitingItems()` (sesión 16), acá no hizo falta un caso especial "sin fecha": todas las quests tienen al menos `createdAt`.
- Alcance: acotado a `renderQuests()` (la vista/pestaña Quests). La sección "Quests activas" del Dashboard (`renderDashboard()`, misma métrica rediseñada en D2/sesión 16) es un componente distinto con su propio pipeline (`quests.filter(q => q.status==='activa')` inline) y no se tocó — no fue lo pedido, y unificarlo es un cambio de alcance mayor si se decide más adelante.
- Cómo se verificó: mismo arnés `jsdom` + `http.server` local de las sesiones anteriores. Dataset: 3 quests activas creadas en orden conocido sin editar (confirma sort por `createdAt` desc), luego se editaron dos de ellas vía el flujo real `editQuest()` → `saveQuest()` (confirma que saltan al frente según el momento real de edición, y que el resto conserva su orden relativo entre sí), más una quest pausada nueva agregada directo a `localStorage` (confirma que el filtro de estado default `'activa'` la excluye pese a ser cronológicamente la más nueva, y que al cambiar a la pill "Todas" aparece rankeada correctamente frente a otra quest nunca editada). Se sumó control de regresión (la card de Quest conserva su `onclick` de navegación y el `stopPropagation` de `card-actions` de la sesión anterior) y smoke test de las otras 6 vistas. 11 checks, todos correctos.
- Hallazgo de §8 resuelto: ninguno (mejora de UX pedida por el usuario, no un bug del análisis estático original — mismo criterio que sesiones 6/11/17).
- Qué quedó pendiente: nada identificado como pendiente de este cambio en sí. Nota fuera de alcance: ni las notas globales ni las de quest (`renderGlobalNotesList()`/`renderQuestNotesList()`) ordenan por `updatedAt` pese a mostrarlo como fecha (`fmtDate(n.updatedAt)`) — quedan en orden de inserción, con las nuevas al frente solo por el `unshift()` de `newQuestNote()`/equivalente global, no por un sort dinámico. No se tocó por no ser lo pedido, pero si se quiere extender "últimas editadas primero" a notas, el mismo comparador aplicaría casi sin cambios. Sigue abierto todo lo de sesiones anteriores: Q5-quests (bloqueado por decisión de producto, sesión 15), §8.7 (evento `DOMSubtreeModified` obsoleto), autosave de tags en notas de quest/globales (sesión 7/8), tooling/portafolio (§8.8-10), concurrencia de auto-release (sesión 12).

### 2026-07-11 (sesión 17)
- Qué cambió: pedido del usuario — el botón de editar tarea (ícono "✎", `.btn-icon`, `padding:5px 10px`) era el único punto de la card donde se podía abrir la edición, y resultaba chico e incómodo (especialmente en mobile). Se aplicó el mismo patrón que ya usan las cards de Quest desde D1 (sesión 13): `onclick="editTask('${t.id}')"` en el `<div class="card">` completo, y `onclick="event.stopPropagation()"` en el `.card-actions` que envuelve ✎/✕, para que ambos botones sigan funcionando de forma independiente sin disparar también la edición. Acotado a las 2 vistas con CRUD real de tarea — `renderQuestTasks()` (tab Tareas de quest-detail) y `renderTasks()` (vista global de Tareas) — en `app.js`; las otras 3 menciones de tarea (dashboard cuentas ocupadas/esperando tokens, Cuentas) son de solo lectura, sin botón editar, y no se tocaron. No hizo falta CSS nuevo: `.card` ya trae `cursor:pointer` global (heredado del mismo patrón de Quest), así que estas cards ya se veían clickeables — antes de este cambio, tocarlas fuera del ícono simplemente no hacía nada.
- Cómo se verificó: mismo arnés `jsdom` + `http.server` local de las sesiones anteriores, contra `index.html`/`app.js` reales. Dataset: 1 quest, 1 cuenta, 2 tareas (una con título, una sin, para cubrir el fallback "Tarea #N"). 13 checks: click en el cuerpo de la card abre `editTask` del id correcto en ambas vistas (con título y con fallback), click en ✎ sigue disparando `editTask` exactamente una vez (no se duplica por el nuevo handler de la card), click en ✕ dispara `deleteTask` y NO dispara `editTask` (el `stopPropagation` corta la burbuja antes de llegar a la card), control de regresión confirmando que `card-actions` de Quest sigue con su `stopPropagation` intacto, y smoke test de las otras 5 vistas (Dashboard/Cuentas/Notas/Stats/Settings) sin excepciones. 13 checks, todos correctos.
- Hallazgo de §8 resuelto: ninguno directamente (mejora de UX pedida por el usuario, no un bug del análisis estático original — mismo criterio que sesiones 6/11), aunque destapa una inconsistencia visual preexistente no listada en §8: el `cursor:pointer` global de `.card` ya prometía que estas cards eran clickeables desde que se agregó (sesión 13, D1), pero hasta ahora no hacían nada fuera del ícono.
- Qué quedó pendiente: nada identificado como pendiente de este cambio en sí. Nota fuera de alcance: las 3 vistas de solo lectura (dashboard, Cuentas) ya usan su propio patrón de click-to-navigate (D1/D2) en vez de click-to-edit — son componentes distintos, no se tocaron y no se decidió si conviene unificar. Sigue abierto todo lo de sesiones anteriores: Q5-quests (bloqueado por decisión de producto, sesión 15), §8.7 (evento `DOMSubtreeModified` obsoleto), autosave de tags en notas de quest/globales (sesión 7/8), tooling/portafolio (§8.8-10), concurrencia de auto-release (sesión 12).

### 2026-07-11 (sesión 16)
- Qué cambió: cuarto lote del plan de cambios (**Fase 4 · Pulido visual + Dashboard**) — tickets **T1** y **D2**, los dos últimos que quedaban del documento.
  - **T1** (`style.css`): la app no tenía una escala tipográfica real — 11 tamaños de `font-size` sueltos repartidos por el archivo (11/12/13×18/14/15/16/18/20/22/32/36px), varios de un único uso sin ningún criterio visible. Se centralizaron los 5 niveles que corresponden a *títulos* en variables nuevas dentro de `:root` (`--fs-eyebrow` 11px, `--fs-label-sm` 13px, `--fs-body-strong` 14px, `--fs-h2` 16px, `--fs-h1` 22px) y se repuntearon `.view-header h1`, `.card-title`, `.account-alias`, `.note-nav-title`, `.section-title`, `.stat-label`, `.settings-section h2`, `.danger-zone h3` y `.modal-header h2` a esas variables. Encontró y corrigió dos inconsistencias reales, no sólo cosméticas:
    - `.settings-section h2` usaba 15px suelto para el mismo rol semántico ("título de sub-sección con nombre propio") que `.modal-header h2` ya resolvía en 16px — se unificaron ambos a `--fs-h2` (único cambio de valor real del ticket; todo lo demás es la misma cifra de antes, ahora como variable).
    - `#qd-title` (el `<h1>` del detalle de quest) no tenía **ninguna** regla propia — no vive dentro de `.view-header`, así que heredaba el `h1` por defecto del navegador: sin `font-family: var(--font-mono)`, sin el peso 700 del resto de los títulos de vista, y con márgenes de bloque (~0.67em arriba/abajo) que además corrían la alineación vertical dentro del flex de `.quest-detail-header`. Se le dio el mismo tratamiento exacto que `.view-header h1` (tamaño, familia, peso, letter-spacing) más `margin:0`.
    - Qué se dejó afuera a propósito: `.note-title-input` (18px, el input editable del título de una nota) no se tocó — no es un título de vista sino un campo de edición, y Notas no está entre las vistas que nombra el ticket (Quest, Cuentas, Dashboard).
  - **D2** (`index.html`, `app.js`, `style.css`): rediseño del dashboard. Antes eran 4 secciones de estado + accesos rápidos, todas visibles a la vez y agrupadas por tipo de dato (cuentas / quests / tareas por separado) — "esperando tokens" además estaba partido en dos (tareas por un lado, cuentas sin tarea por otro) pese a ser el mismo concepto para quien mira el tablero. Ahora son 3 métricas protagonistas (números siempre visibles, reusando el mismo `.stat-card` de la vista Stats) y el detalle de cada una se expande bajo demanda:
    - **Esperando tokens**: nuevo helper `buildDashWaitingItems()` combina tareas `esperando_tokens` + cuentas `esperando_tokens` sin tarea en una sola lista ordenada por fecha de reactivación (más próxima primero; `reactivation`/`waitReactivation` no son obligatorios — ver `saveTask()`/`saveAccountWait()` —, así que un item sin fecha va al final en vez de romper el sort). El sub-label de la métrica ya adelanta la fecha más próxima sin tener que expandir la lista.
    - **Quests activas**: mismo listado de antes, con un sub-label nuevo (cantidad de tareas no completadas en esas quests).
    - **Cuentas ocupadas**: mismo listado de antes, con un sub-label nuevo (cuántas cuentas están libres ahora mismo) — complementa la métrica sin agregar una 4ta card protagonista.
    - Expand/collapse (`toggleDashGroup()`) no guarda el estado "abierto" en ninguna variable: como `showView()` sólo alterna `.active` (nunca destruye `#view-dashboard`) y `renderDashboard()` sólo reescribe el innerHTML de los contenedores internos (nunca la clase `.open` de `.dash-group` ni el `aria-expanded` del botón), el estado sobrevive solo tanto a un re-render de fondo (`checkAutoReleases()` llama a `renderDashboard()` directo cada 30s si `currentView==='dashboard'`) como a navegar a otra vista y volver.
    - El color de borde de cada métrica (`--warn`/`--accent`/`--muted`) reusa el mismo lenguaje que ya usa `.account-card` para status — no depende de Q5-quests (ver pendientes).
    - D1 (fix de navegación, resuelto en sesión 13) queda intacto: se preservó el mismo patrón `onclick="showView(...)"` en las 3 variantes de card, sólo cambiaron los contenedores donde se insertan.
- Cómo se verificó: arnés `jsdom` (mismo enfoque de sesiones anteriores: `http.server` local + `index.html`/`app.js` reales, ya que `localStorage` no está disponible para el origen `file://`). Dataset con 5 cuentas (libre / ocupada con tarea en curso / ocupada con tarea esperando tokens / esperando tokens sin tarea con fecha / esperando tokens sin tarea sin fecha) y 2 quests (activa con 3 tareas en distintos estados, pausada) para cubrir: conteo y sub-label de las 3 métricas, orden de la lista unificada de espera (fecha más próxima primero, sin fecha al final), que una cuenta ocupada cuya tarea está esperando tokens sigue mostrando el badge de reactivación en su propia card (doble listado intencional, ya se comportaba así antes de esta sesión), colapsado por defecto de los 3 grupos, toggle abre/cierra y actualiza `aria-expanded`, el estado abierto sobrevive a un `showView('dashboard')` repetido (equivalente al refresco que dispara `checkAutoReleases()`) y a navegar a otra vista y volver, click de las 3 variantes de card (D1) sigue navegando a la vista correcta, estados vacíos con la DB limpia, y smoke test de las otras 6 vistas. Para T1: `#qd-title` iguala tamaño/familia/margen a `.view-header h1`, y `.settings-section h2`/`.modal-header h2` citan ahora la misma variable (jsdom no resuelve `var()` dentro de `getComputedStyle` — se confirmó el valor final de `--fs-h2` a nivel de archivo fuente aparte). 46 checks, todos correctos.
- Hallazgo de §8 resuelto: ninguno directamente (T1 y D2 vienen del plan externo, no de este listado), pero el bug real que encontró T1 en `#qd-title` es de la misma familia que varios hallazgos anteriores (sesión 13/Q3, sesión 8): un elemento que se asume estilado porque *se ve parecido* a otro, sin haber sido verificado contra el CSS real.
- Qué quedó pendiente: **Q5-quests** (heredar color de categoría vs. color libre por quest) sigue bloqueado por la misma decisión de producto sin resolver que en sesión 15 — D2 usa sólo el lenguaje de color por status que ya existía (`--warn`/`--accent`/`--muted`), sin depender de esa decisión. Con esto se completan las 4 fases del plan de cambios externo; sólo queda Q5-quests, a la espera de la decisión de producto, y todo lo de sesiones anteriores que seguía abierto (§8.7, autosave de tags en notas de quest/globales, tooling/portafolio §8.8-10, concurrencia de auto-release de sesión 12).
